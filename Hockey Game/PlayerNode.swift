...
open class UserPlayerNode: SKNode, PlayerNodeDelegate {
    ...
    public init(withColor color: SKColor = .white, rinkReference: Rink, andPosition playerPosition: PlayerPosition) {
        ...
        self.setPhysicsBody(withTexture: self.playerNode.texture!)
    }
    ...
    open func setPhysicsBody(withTexture texture: SKTexture) {
        let physicsBody = SKPhysicsBody(texture: texture, size: playerNode.size)
        self.physicsBody = physicsBody
        self.physicsBody?.isDynamic = true
        self.physicsBody?.mass = 1
        self.physicsBody?.categoryBitMask = PhysicsCategory.player
    ...
    func playerNodeDidStartSkating(_ node: PlayerNode) {
        self.setPhysicsBody(withTexture: PlayerTexture.f1)
    }
    
    func playerNodeDidStopSkating(_ node: PlayerNode) {
        self.setPhysicsBody(withTexture: PlayerTexture.faceoff)
    }
