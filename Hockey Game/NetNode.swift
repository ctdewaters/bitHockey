...
public class NetNode: SKSpriteNode {
    ...
    public init(atRinkEnd rinkEnd: RinkEnd) {
        //Load textures
        super.init(texture: SKTexture(imageNamed: "netTexture"), color: SKColor.clear, size: CGSize(width: netWidth, height: netDepth))
    ...
    fileprivate func setPhysicsBody() {
        self.physicsBody = SKPhysicsBody(texture: PlayerTexture.netPhysicsBody, size: self.size)
        self.physicsBody?.restitution = 0.1
        self.physicsBody?.allowsRotation = false
        self.physicsBody?.isDynamic = false
        self.physicsBody?.categoryBitMask = PhysicsCategory.net
    }
